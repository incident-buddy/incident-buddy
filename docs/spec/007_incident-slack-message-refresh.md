...
- `api/src/slack/handlers/actions.ts`
  - `refreshIncidentSlackMessage(incidentId: string, client: WebClient): Promise<void>` 関数を追加・エクスポートする
  - 既存の `registerActionHandlers` 関数には変更なし
...
export async function refreshIncidentSlackMessage(
  ...
  if (!incident.slackMessageTs) return; // 未投稿ならスキップ

...
export class RuleEvaluator {
  ...
  async evaluate(
    ...
    const firstNonDeterministicIndex =
      this.findFirstNonDeterministicRuleIndex();
    const leadingDeterministicIndex =
      this.evaluateImmediateDeterministicConditions(
        0,
        firstNonDeterministicIndex >= 0
          ? firstNonDeterministicIndex
          : undefined,
      );
    ...
    const phase3TagIndex = this.resolveTaggedRuleIndex(
      tagContent,
      interactiveEnabled,
    );
    if (phase3TagIndex >= 0) {
      const immediateDeterministicIndex =
        this.evaluateImmediateDeterministicConditions(
    ...
    const phase1TagIndex = this.resolveTaggedRuleIndex(
      agentContent,
      interactiveEnabled,
    );
    if (phase1TagIndex >= 0) {
    ...
    const fallbackIndex =
      await this.evaluateAllConditionsViaAiJudge(agentContent);
    if (fallbackIndex >= 0) {
      return { index: fallbackIndex, method: "ai_judge_fallback" };
    }

    const deferredDeterministicIndex =
      this.evaluateDeferredDeterministicConditions();
    if (deferredDeterministicIndex >= 0) {
      return { index: deferredDeterministicIndex, method: "auto_select" };
    }

    throw new Error(
      `Status not found for step "${this.step.name}": no rule matched after all detection phases`,
    );
  }

  private resolveTaggedRuleIndex(
    content: string,
  ...
  private evaluateImmediateDeterministicConditions(
    startIndex = 0,
    endExclusive?: number,
  ): number {
    if (!this.step.rules) return -1;
  ...
  private evaluateDeferredDeterministicConditions(): number {
    ...
    for (let i = 0; i < this.step.rules.length; i++) {
      ...
      if (
        ...
        !isDeferredDeterministicCondition(rule.condition)
      ) {
        continue;
      }
      if (evaluateWhenExpression(rule.condition, this.ctx.state)) {
  ...
  private async evaluateAiConditions(agentOutput: string): Promise<number> {
    ...
    const matched = aiConditions.find(
      (condition) => condition.index === judgeResult,
    );
    if (matched) {
   ...
   * Returns the 0-based rule index, or -1 if no match.
   */
  private async evaluateAllConditionsViaAiJudge(
    agentOutput: string,
  ): Promise<number> {
    ...
    const judgeableRules = this.step.rules
      ...
      .filter(
        ({ rule }) =>
          !rule.isAggregateCondition &&
          !isDeterministicCondition(rule.condition),
      )
      .filter(
        ({ rule }) => this.ctx.interactive === true || !rule.interactiveOnly,
      );

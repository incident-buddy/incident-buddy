...
import { buildUsageEventRecord, type StepType } from "./providerEvent.js";
...
export function createUsageEventLogger(
  config: UsageEventLoggerConfig,
): UsageEventLogger {
  if (config.enabled) {
  ...
  const filepath = join(
    config.logsDir,
    `${config.sessionId}${USAGE_EVENTS_LOG_FILE_SUFFIX}`,
  );
    ...
    logUsage(params: {
      ...
      try {
        ...
        process.stderr.write(
          `[takt] Failed to write usage event log: ${message}\n`,
        );
      }
    },
  };

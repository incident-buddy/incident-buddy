...
## D: インシデントチャンネルへのウェルカムメッセージ投稿

インシデントチャンネル作成後、そのチャンネル内にインシデント詳細（title, severity, service, description, 宣言者）を自動投稿する。

**なぜここか**: チャンネルに入ったメンバーが「何のインシデントか」をすぐ把握できるようになる。E の次に高い即時価値。

**実装のポイント**:
- `incident.presenter.ts` にチャンネル内投稿用メッセージビルダーを追加
- `actions.ts` でチャンネル作成・招待後に `chat.postMessage` をチャンネルに投稿

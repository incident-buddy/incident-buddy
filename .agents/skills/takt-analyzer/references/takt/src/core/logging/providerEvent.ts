...
function truncateString(value: string): string {
  ...
  return (
    value.slice(0, HEAD_LENGTH) + TRUNCATED_MARKER + value.slice(-TAIL_LENGTH)
  );
}
...
export function normalizeProviderEvent(
  ...
  const messageId = pickString(data, [
    "message_id",
    "messageId",
    "item_id",
    "itemId",
...
export function buildUsageEventRecord(
  ...
  if (params.usage.usageMissing) {
    if (
      typeof params.usage.reason !== "string" ||
      params.usage.reason.length === 0
    ) {
      throw new Error(
        "[usage-events] reason is required when usageMissing=true",
      );
    }
    return {
      run_id: meta.runId,

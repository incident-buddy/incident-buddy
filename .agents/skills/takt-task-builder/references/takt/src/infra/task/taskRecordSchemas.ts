...
export const TaskRecordSchema = buildTaskSchema(
  ...
  if (value.status === "exceeded") {
    ...
    if (hasExceededMax !== hasExceededIter) {
      ctx.addIssue({
        ...
        message:
          "exceeded_max_steps and exceeded_current_iteration must both be set or both be absent.",
      });
    }
  }
...
export function serializeTasksFileData(state: TasksFileData): {

...
export function createProviderEventLogger(
  config: ProviderEventLoggerConfig,
): ProviderEventLogger {
  if (config.enabled) {
  ...
  const filepath = join(
    config.logsDir,
    `${config.sessionId}${PROVIDER_EVENTS_LOG_FILE_SUFFIX}`,
  );
  ...
  const write = (event: StreamEvent): void => {
    ...
    try {
      ...
      process.stderr.write(
        `[takt] Failed to write provider event log: ${message}\n`,
      );
    }
  };

...
function withConfigFile({
  ...
  return async () => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), "incident-channel-test-"),
    );
...
function buildCreateIncidentPayload(
  opts: { severity?: string; channelId?: string } = {},
) {
...
describe("インシデントチャンネル自動作成・招待", () => {
  ...
  describe("ユーザー招待", () => {
    ...
    it(
      ...
      withConfigFile({
        content: CONFIG_WITH_GROUP_MENTION,
        fn: async () => {
          let capturedInviteUsers: string | undefined;
          let resolveInvite!: () => void;
          const inviteCalled = new Promise<void>((r) => {
    ...
    it(
      ...
      withConfigFile({
        content: CONFIG_WITH_DUPLICATE_USERS,
        fn: async () => {

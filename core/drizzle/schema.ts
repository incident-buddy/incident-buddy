...
export const userHashedPasswords = pgTable(
	"user_hashed_passwords",
	{
		id: text().primaryKey().notNull(),
		userId: text("user_id").notNull(),
...
export const resourceMasters = pgTable(
	"resource_masters",
	{
		id: text().primaryKey().notNull(),
		name: text().notNull(),
	...
	(table) => [
		uniqueIndex("resource_masters__ti_co").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
...
export const resourceMasterCategories = pgTable(
	"resource_master_categories",
	{
		id: text().primaryKey().notNull(),
	...
	(table) => [
		index("resource_master_categories__ti_cc_rm").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.categoryCode.asc().nullsLast().op("text_ops"),
			table.resourceMasterId.asc().nullsLast().op("text_ops"),
		),
		foreignKey({
...
export const resourceMasterAttributes = pgTable(
	"resource_master_attributes",
	{
		id: text().primaryKey().notNull(),
		name: text().notNull(),
		code: text().notNull(),
	...
	(table) => [
		index("resource_master_attributes__rm_on").using(
			"btree",
			table.resourceMasterId.asc().nullsLast().op("int4_ops"),
...
export const resources = pgTable(
	...
	(table) => [
		index("resources__ti_rm_co").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.resourceMasterId.asc().nullsLast().op("text_ops"),
			table.code.asc().nullsLast().op("text_ops"),
...
export const resourceAttributeValues = pgTable(
...
export const incidentStatuses = pgTable(
	"incident_statuses",
	{
		id: text().primaryKey().notNull(),
		name: text().notNull(),
...
export const incidentRoles = pgTable(
	"incident_roles",
	{
		id: text().primaryKey().notNull(),
		name: text().notNull(),
...
export const incidentRoleSlots = pgTable(
	"incident_role_slots",
	{
		id: text().primaryKey().notNull(),
	...
	(table) => [
		index("incident_role_slots__ti_ii_ri").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.incidentId.asc().nullsLast().op("text_ops"),
...
export const incidentRoleAssignments = pgTable(
	...
	(table) => [
		index("incident_role_assignments__ti_ii_rsi").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.incidentId.asc().nullsLast().op("text_ops"),
			table.roleSlotId.asc().nullsLast().op("text_ops"),
		),
		foreignKey({
			columns: [table.incidentId],
			foreignColumns: [incidents.id],
...
export const incidentEventHistories = pgTable(
	"incident_event_histories",
	{
		id: text().primaryKey().notNull(),
		incidentId: text("incident_id").notNull(),
	...
	(table) => [
		index("incident_event_histories__ti_ii_et_ra").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.incidentId.asc().nullsLast().op("text_ops"),
			table.eventType.asc().nullsLast().op("timestamp_ops"),
			table.placedAt.asc().nullsLast().op("text_ops"),
		),
		index("incident_event_histories_ti_ii_ep_ecp_eci").using(
			"btree",
			table.tenantId.asc().nullsLast().op("text_ops"),
			table.incidentId.asc().nullsLast().op("text_ops"),
			table.externalPlatform.asc().nullsLast().op("text_ops"),
			table.externalPlace.asc().nullsLast().op("text_ops"),
...
export const workflowExecutions = pgTable(
	...
	(table) => [
		index("workflow_executions__ti_sa_s").using(
			"btree",
